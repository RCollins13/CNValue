...
#Code to generate barplots of significant elements by pheno group (Fig 3)
...
#####Read data
##############
df <- read.table(paste(WRKDIR,"plot_data/NoncodingElementClassesFigure/significant_loci_per_pheno.del_dup_both.counts.txt",sep=""),header=F)
names(df) <- c("pheno","DEL","BOTH","DUP")
df <- df[phenos.reorder,]
#Subset to just non-cancer phenotypes
df <- df[1:22,]
df$SUM <- df$DEL+df$BOTH+df$DUP
df.sumOrder <- order(df$SUM,decreasing=T)
# df[,-1] <- apply(df[,-1],2,function(vals){
#   nVals <- log10(vals)
#   nVals[which(is.infinite(nVals))] <- 0
#   return(nVals)
# })
...
#####Plot bars
##############
pdf(paste(WRKDIR,"rCNV_map_paper/Figures/NoncodingElementClasses/signif_loci_counts_per_pheno.barplots.pdf",sep=""),
    width=2.5,height=4)
plotBars(df)
dev.off()
