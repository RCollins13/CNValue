...
outfile=$7
exclude=$8 #list of genes to exclude from all analyses
...
while read ID list; do
  for dummy in 1; do
    ...
    fgrep -v "#" ${universe} sed 's/\-/_/g' | \
    fgrep -wvf <( sed 's/\-/_/g' ${exclude} ) | wc -l
    #Get number of genes in list in universe
    fgrep -wf <( sed 's/\-/_/g' ${list} ) <( sed 's/\-/_/g' ${universe} ) | \
    fgrep -wvf <( sed 's/\-/_/g' ${exclude} ) | wc -l
