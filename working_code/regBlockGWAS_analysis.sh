...
#Code to identify noncoding CNV hotspots for rare CNV project
#GWAS-style approach for noncoding reg block prioritization
#(Rather than element burden-test-based method)
...
##############################################
#Create master exclusion masks for DEL and DUP
##############################################
#DEL mask:
# - no N-masked bases
# - 100kb buffer for telomeres
# - 1Mb buffer for centromeres
# - no gene bodies of not-haplosufficient genes
# - no gene bodies of DEL-significant genes from per-gene based test
# - no CTRL probe deserts
...
${WRKDIR}/data/master_annotations/gencode/gencode.v19.gene_boundaries.notHaplosufficient.bed \
<( fgrep -wf ${WRKDIR}/analysis/perGene_burden/signif_genes/merged/MasterPhenoGroups_DEL_E4_exonic.geneScore_FINAL_sig.genes.list \
   <( sed -e 's/\-/_/g' -e 's/\./_/g' \
...
${WRKDIR}/data/master_annotations/other/regBlockHotspotAnalysis.excluded_loci.DEL.bed
#DUP mask:
# - no N-masked bases
# - 100kb buffer for telomeres
# - 1Mb buffer for centromeres
# - no gene bodies of protein-coding genes
# - no CTRL probe deserts
cat <( grep -e 'telomere' /data/talkowski/rlc47/src/GRCh37_heterochromatin.bed | \
awk -v OFS="\t" '{ print $1, $2-100000, $3+100000 }' | awk -v OFS="\t" '{ if ($2<0) $2=0; print }' ) \
<( grep -e 'centromere' /data/talkowski/rlc47/src/GRCh37_heterochromatin.bed | \
...
${WRKDIR}/data/master_annotations/gencode/gencode.v19.gene_boundaries.protein_coding.bed | \
cut -f1-3 | sort -Vk1,1 -k2,2n -k3,3n | bedtools merge -i - > \
${WRKDIR}/data/master_annotations/other/regBlockHotspotAnalysis.excluded_loci.DUP.bed

################
#Run CNV pileups
################
#10kb bins, 10kb steps, 10kb window, no window smoothing
#Clear directory
if [ -e ${WRKDIR}/analysis/regBlockGWAS_pileups ]; then
  rm -rf ${WRKDIR}/analysis/regBlockGWAS_pileups
fi
mkdir ${WRKDIR}/analysis/regBlockGWAS_pileups
#Launch deletion jobs
CNV=DEL; VF=E4; filt=haplosufficient
for pheno in CTRL GERM NEURO NDD PSYCH SOMA; do
  #Create directory, if necessary
  if ! [ -e ${WRKDIR}/analysis/regBlockGWAS_pileups/${pheno} ]; then
    mkdir ${WRKDIR}/analysis/regBlockGWAS_pileups/${pheno}
  fi
  #Parallelize intersections
  bsub -q short -sla miket_sc -u nobody -J ${pheno}_${CNV}_${VF}_${filt}_regBlockGWAS_binned_pileup \
  "${WRKDIR}/bin/rCNVmap/bin/TBRden_binned_pileup.sh -z -w 10000 -s 10000 -d 10000 -I 0.01 \
  -o ${WRKDIR}/analysis/regBlockGWAS_pileups/${pheno}/${pheno}.${CNV}.${VF}.${filt}.regBlockGWAS_pileup.bed \
  -x ${WRKDIR}/data/master_annotations/other/regBlockHotspotAnalysis.excluded_loci.${CNV}.bed  \
  ${WRKDIR}/data/CNV/CNV_MASTER/${pheno}/${pheno}.${CNV}.${VF}.GRCh37.${filt}.bed.gz \
  /data/talkowski/rlc47/src/GRCh37.genome"
done
#Launch duplication jobs
CNV=DUP; VF=E4; filt=noncoding
for pheno in CTRL GERM NEURO NDD PSYCH SOMA; do
  #Create directory, if necessary
  if ! [ -e ${WRKDIR}/analysis/regBlockGWAS_pileups/${pheno} ]; then
    mkdir ${WRKDIR}/analysis/regBlockGWAS_pileups/${pheno}
...
if [ -e ${WRKDIR}/analysis/regBlockGWAS_burdens ]; then
  rm -rf ${WRKDIR}/analysis/regBlockGWAS_burdens
fi
mkdir ${WRKDIR}/analysis/regBlockGWAS_burdens
#Iterate over all phenotypes
ncontrol=38628
while read pheno color ncase; do
  #Create directory, if necessary
  if ! [ -e ${WRKDIR}/analysis/regBlockGWAS_burdens/${pheno}/ ]; then
    mkdir ${WRKDIR}/analysis/regBlockGWAS_burdens/${pheno}/
  fi
  #DEL jobs
  CNV=DEL; VF=E4; filt=haplosufficient
  bsub -q short -sla miket_sc -J ${pheno}_${CNV}_${VF}_${filt}_pileup_analysis -u nobody \
  -o ${WRKDIR}/analysis/regBlockGWAS_burdens/${pheno}/${smooth}kb_smoothed/${pheno}_${CNV}_${filter}.out \
  -e ${WRKDIR}/analysis/regBlockGWAS_burdens/${pheno}/${smooth}kb_smoothed/${pheno}_${CNV}_${filter}.err \
  "${WRKDIR}/bin/rCNVmap/bin/TBRden_test.R \
  ${WRKDIR}/analysis/regBlockGWAS_pileups/CTRL/CTRL.${CNV}.${VF}.${filt}.regBlockGWAS_pileup.bed.gz \
  ${WRKDIR}/analysis/regBlockGWAS_pileups/${pheno}/${pheno}.${CNV}.${VF}.${filt}.regBlockGWAS_pileup.bed.gz \
  ${WRKDIR}/analysis/regBlockGWAS_burdens/${pheno}/ \
